...
async function sendEmail({ to, from, replyTo, subject, html }) {
  ...
  const r = await fetch('https://api.resend.com/emails', { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from, to: Array.isArray(to) ? to : [to], reply_to: replyTo, subject, html }) });
...
module.exports = { sanitize, isRateLimited, isGibberish, hashIp, getSupabase, sendEmail, NOTIFY_TO, SEND_FROM, hashPassword, verifyPassword, signSession, verifySession, parseCookies, SESSION_COOKIE };
